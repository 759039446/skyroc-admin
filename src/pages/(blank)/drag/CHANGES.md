...
### 最新更新：企业权益真正动态自适应系统 (2024-11-30) ⭐⭐⭐ 重大升级
- **新增功能**：企业权益组件支持动态网格布局配置
- **每行列数配置**：
  - 可选择2、3、4、5、6、8列布局
  - 默认6列显示
  - 自动计算项目宽度
- **革命性改进：完全动态自适应** ⭐⭐⭐ 
  ...
  - ✅ 根据容器尺寸动态计算所有元素大小
  - ✅ 支持任意尺寸的容器，无缝适配
  - ✅ 使用ResizeObserver监听尺寸变化，实时响应

- **智能计算系统** ⭐⭐⭐ 核心技术：
  - **项目宽度计算**：`(容器宽度 - 所有间距) / 列数`
  - **项目高度计算**：自动或手动，支持基于容器高度自动分配
  - **图标大小**：基准尺寸的40%（20-60px范围）
  - **标题大小**：项目宽度的10%（12-22px范围）
  - **描述大小**：标题的75%（10px起）
  - **内边距**：基准尺寸的10%（8-24px范围）
  - **内部间距**：基准尺寸的7%（6-14px范围）
  
- **智能显示策略** ⭐⭐ 
  - 当项目宽度 ≥ 120px：显示图标 + 标题 + 描述
  - 当项目宽度 < 120px：显示图标 + 标题，自动隐藏描述
  - 根据实际空间自动决定，无需配置
...
- **用户文档**：
  - `README.md` - 完整使用说明
  - `LAYOUT_GUIDE.md` - 布局效果指南
  - `SIZE_GUIDE.md` - 尺寸自适应详解 ⭐ 新增

### 主导航菜单三级分类系统 (2024-11-29)
- **重大升级**：主导航菜单支持完整的三级分类体系
- **数据分离**：分类数据提取到 `categories.json` 文件，便于维护和扩展
- **样式分离**：滚动条样式提取到 `styles.css` 文件，代码更清晰
- **滚动条美化升级**：全新的滚动条设计，更现代更精致
- **布局改进**：
  - **大分类**：竖向排列，占据组件全部高度
  - **子菜单**：固定宽度浮窗，紧贴组件右侧
  - **浮窗高度**：与主组件高度一致，自动对齐
  - **默认尺寸**：宽度 25%，高度 100 行（500px）
- **三级分类结构**：
  - **一级分类**（大分类）：工具/焊接用品、紧固/密封等
  - **二级分类**（中分类）：手动工具、电动工具等
  - **三级分类**（小分类）：手动扳手、六角扳手等
- **视觉区分**：
  - **一级分类**：15px 字体，悬停加粗显示
  - **二级分类**：16px 粗体，带主题色下划线
  - **三级分类**：13px 常规，灰色背景标签
- **数据结构**：
  ```typescript
  categories: [
...
- **滚动条美化**（全新升级）：
  - **主菜单滚动条**：
    - 宽度 8px，圆角 10px
    - 渐变背景轨道（#fafafa → #f5f5f5）
    - 渐变滑块（#d0d0d0 → #b8b8b8）
    - 2px 白色边框，精致立体
  - **子菜单滚动条**：
    - 宽度 10px，圆角 10px
    - 渐变背景轨道（#f8f8f8 → #f0f0f0）+ 内阴影
    - 渐变滑块（#a8a8a8 → #8a8a8a）
    - 2px 白色边框
  - **悬停效果**：
    - 滑块变为主题色渐变
  ...
  - **技术特点**：
    ...
    - 性能优异，无额外 DOM
- **可配置项**：
  - **子菜单宽度**：300-1200px，默认 600px
  - **背景颜色**：可自定义
  - **悬停颜色**：可自定义（默认红色）
  - **边框颜色**：可自定义
- **配置格式**：
  ```
  一级分类|二级1:三级1,三级2;二级2:三级3,三级4
  ```
  示例：
  ```
...
- **丰富数据**：
  ...
  - 总计超过 150 个三级分类项
- **数据管理**：
  - **数据文件**：`components/shop/MainNavMenu/categories.json`
  - **样式文件**：`components/shop/MainNavMenu/styles.css`
  - **文档说明**：`components/shop/MainNavMenu/README.md`
  - **滚动条文档**：`components/shop/MainNavMenu/SCROLLBAR.md`
  - **导入方式**：
    - 数据：`import defaultCategories from './categories.json'`
    - 样式：`import './styles.css'`
  - **易于维护**：
    - 直接编辑 JSON 文件更新分类数据
    - 直接编辑 CSS 文件自定义滚动条样式
  - **格式验证**：JSON 格式便于验证和版本控制
  - **代码分离**：数据、样式、逻辑完全分离
- **技术实现**：
  - CSS 文件导入，样式与逻辑分离
  - fixed 定位浮窗，动态计算位置和高度
  - Flexbox 布局，三级分类自动换行
  - React hooks 管理交互状态
  - JSON 模块导入，支持热更新
  - Webpack 自动处理 CSS 模块
...
### 画布网格显示与无留白设计 (2024-11-29)
- **新增功能**：画布上显示网格线，辅助组件对齐和布局
- **网格特性**：
  - 根据 `gridConfig.cols` 和 `gridConfig.rowHeight` 动态计算网格大小
  - 网格列宽 = 画布宽度 / 列数
  - 网格行高 = 配置的行高值
  - 使用 CSS `linear-gradient` 绘制，性能优异
  - 当前配置：96列 × 5px行高 = 12.5px × 5px 网格
- **显示规则**：
  - **设计模式**：显示网格线（浅灰色 #e8e8e8）
  - **预览模式**：不显示网格线，呈现最终效果
- **无留白设计**：
  - **设计模式画布**：去掉所有内外边距，组件从 (0, 0) 开始
  - **预览模式画布**：同样去掉所有留白，与设计模式保持一致
  - **统一体验**：设计即所见，预览即最终效果
- **技术实现**：
  - 通过内联样式动态设置 `backgroundImage` 和 `backgroundSize`
  - 预览模式使用独立样式类，不包含网格背景
  - 画布容器和画布本身的 padding 均设为 0
  - 去掉圆角和阴影效果，纯净展示
- **用户价值**：
  - 精准对齐组件位置
  - 可视化网格系统
...
### 设计模式与预览模式 (2024-11-29)
- **新增功能**：所有组件支持设计模式和预览模式切换
- **设计模式**：在画布上点击组件时，组件内部的点击事件被禁用，避免干扰布局设计
- **预览模式**：点击"预览"按钮后，组件内部的所有交互功能正常工作
- **技术实现**：
  - 为所有组件的 `render` 函数添加 `isDesignMode` 参数
  - 使用 CSS `pointerEvents` 属性控制组件交互
  - 在画布渲染时默认传递 `isDesignMode=true`
- **涵盖组件**：
  - 8个基础组件（按钮、输入框、卡片、标签、文本、标题、容器、图片）
  - 11个商城组件（导航栏、搜索、购物车、Logo、菜单、广告、企业权益、促销、商品推荐、底部信息、认证）
- **用户体验**：
  - 设计布局时不会误触组件功能
  - 预览时可以完整测试所有交互
  - 无缝切换两种模式
...
23. **src/pages/(blank)/drag/components/DESIGN_MODE.md**
    - 设计模式与预览模式说明文档
    - 详细说明两种模式的区别和用途
    - 技术实现方式和使用指南
    - 开发指南：如何为新组件添加支持
...
- **设计模式特点**：
  ...
  - 可以调整组件大小
  - 可以选中组件进行配置
  - 组件内部点击事件被禁用（`pointerEvents: 'none'`）
- **预览模式特点**：
  - 组件内部交互功能正常工作
  - 可以测试按钮、表单等功能
  - 真实模拟最终效果
